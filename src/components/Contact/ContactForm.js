...
const ContactForm = () => {
   ...
   const { loading, setLoading, setShowSnackbar } = useContext(uiContext);
               ...
               {textfielDetails.map((item, index) => (
                  <Box sx={{ width: '100%' }} key={index}>
                     <TextField
                        ...
                        variant="outlined"
                        sx={{
                           background: errors[item.name] ? '#fff0f0' : '#f8f6fe',
                           width: '100%',
                           '& .MuiOutlinedInput-root': {
                              ...
                              '&.Mui-focused fieldset': {
                                 borderColor: errors[item.name] ? 'red' : '#784cfb',
                              },
                           },
                        }}
                        InputLabelProps={{
                           style: {
                              color: errors[item.name] ? '#ff6666' : 'rgba(120, 76, 251, 0.75)',
